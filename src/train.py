...
class DiffusionSampler(nn.Module):
    ...
    def p_sample(self, model: nn.Module, xt: Tensor, t: Tensor) -> Tensor:
        ...
        var = beta * (1.0 - alpha_bar_prev)
        mu = (xt - epsilon * beta / (1.0 - alpha_bar).sqrt()) / alpha.sqrt()

        sample = (mu + not_first_timestep * var.sqrt() * z)
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((32, 32)),  # <<< REQUIRED FIX
        transforms.ToTensor(),
    ])
    mnist = tvdatasets.MNIST(root="./data", train=True, download=True, transform=transform)
    loader = torch.utils.data.DataLoader(mnist, batch_size=batch_size, shuffle=True)


    sampler = DiffusionSampler(timesteps, 'cosine').to(device)
    ...
    for i in range(steps):
        ...
        x0, _ = next(iter(loader))  # MNIST batch
        x0 = x0.to(device)
        ## compute loss
